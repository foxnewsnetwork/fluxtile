...
tests.PowerSurferMonsterTest = function() { }
tests.PowerSurferMonsterTest.__name__ = ["tests","PowerSurferMonsterTest"];
tests.PowerSurferMonsterTest.main = function() {
	var psm = new monsters.PowerSurfer();
}
tests.PowerSurferMonsterTest.prototype.__class__ = tests.PowerSurferMonsterTest;
...
monsters.Monster = function(p) {
	if( p === $_ ) return;
	var me = this;
	buildingblocks.Tile.call(this);
	this.ui = new Hash();
	this.inputs = new Array();
	var _g = 0, _g1 = ["submit","new","close"];
	while(_g < _g1.length) {
		var k = _g1[_g];
		++_g;
		var t = [new buildingblocks.Tile()];
		t[0].HTML(k);
		t[0].CSS("border","1px solid blue");
		t[0].Mouseover((function(t) {
			return function(e) {
				t[0].CSS("border","2px solid red");
			};
		})(t));
		t[0].Mouseleave((function(t) {
			return function(e) {
				t[0].CSS("border","1px solid blue");
			};
		})(t));
		this.ui.set(k,t[0]);
		t[0].Size({ width : 70.0, height : 55.0});
	}
	this.ui.get("close").Click(function(e) {
		me.Hide();
	});
	this.ui.get("new").Click(function(e) {
		var nid = "name-" + monsters.Monster.NAME + monsters.Monster.ID;
		var vid = "value-" + monsters.Monster.NAME + monsters.Monster.ID;
		var context = "";
		context += "<h3>";
		...
		context += "</h3>";
		me.Prepend(context);
		me.inputs.push({ name : new js.JQuery("#" + nid), value : new js.JQuery("#" + vid)});
...
monsters.Monster.__name__ = ["monsters","Monster"];
monsters.Monster.__super__ = buildingblocks.Tile;
for(var k in buildingblocks.Tile.prototype ) monsters.Monster.prototype[k] = buildingblocks.Tile.prototype[k];
monsters.Monster.prototype.inputs = null;
monsters.Monster.prototype.ui = null;
monsters.Monster.prototype.Hide = function(cb) {
...
monsters.Monster.prototype.Show = function(cb) {
...
monsters.Monster.prototype.__class__ = monsters.Monster;
monsters.PowerSurfer = function(p) {
	if( p === $_ ) return;
	var me = this;
	monsters.Monster.call(this);
	var ctx = "<h3>Action : ";
	ctx += "<input type='text' id='action-" + monsters.Monster.NAME + monsters.Monster.ID + "' value='" + js.Lib.window.location.href + "'/>";
	ctx += "</h3><h3>Method : ";
	ctx += "<input type='text' id='method-" + monsters.Monster.NAME + monsters.Monster.ID + "' value='get' />";
	ctx += "</h3>";
	this.HTML(ctx);
	this.action = new js.JQuery("#action-" + monsters.Monster.NAME + monsters.Monster.ID);
	this.method = new js.JQuery("#method-" + monsters.Monster.NAME + monsters.Monster.ID);
	monsters.Monster.ID += 1;
	this.ui.get("submit").Click(function(e) {
		var req = new haxe.Http(me.action.val());
		var reqData = "";
		var meth = me.method.val();
		if(new EReg("^get","i").match(meth)) {
			var _g = 0, _g1 = me.inputs;
			while(_g < _g1.length) {
				var k = _g1[_g];
				++_g;
		...
		req.onData = function(r) {
			me.result = r;
			haxe.Log.trace(me.result,{ fileName : "PowerSurfer.hx", lineNumber : 42, className : "monsters.PowerSurfer", methodName : "new"});
		};
		req.request(new EReg("^post","i").match(meth));
	});
}
monsters.PowerSurfer.__name__ = ["monsters","PowerSurfer"];
monsters.PowerSurfer.__super__ = monsters.Monster;
for(var k in monsters.Monster.prototype ) monsters.PowerSurfer.prototype[k] = monsters.Monster.prototype[k];
monsters.PowerSurfer.prototype.action = null;
monsters.PowerSurfer.prototype.method = null;
monsters.PowerSurfer.prototype.result = null;
monsters.PowerSurfer.prototype.__class__ = monsters.PowerSurfer;
...
monsters.Monster.NAME = "TILE-MONSTER-" + tools.Random.Get(999999);
monsters.Monster.ID = 0;
tools.Timer.TIME = haxe.Timer.stamp();
js.Lib.onerror = null;
tools.Tooltip.HaxeToolTip = new buildingblocks.Tile();
tools.Tooltip.ID = 0;
animation.Spotlight.Lights = new animation.BoxHighlighter();
tests.PowerSurferMonsterTest.main()